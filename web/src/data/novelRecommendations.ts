export interface NovelRecommendation {
  title: string;
  url: string;
  genre: 'Isekai & Fantasy' | 'Psychological' | 'Mystery & Historical' | 'Rom-Com & Slice of Life' | 'Action & Dungeon';
  description: string;
  basedOn: string;
}
...
export const NOVEL_CATALOG: NovelRecommendation[] = [
  // Isekai & Fantasy
  {
    ...
    description: "The grandfather of modern isekai. A 34-year-old NEET reincarnates into a world of magic with memories intact.",
  ...
  },
  {
    ...
    genre: "Isekai & Fantasy",
    description: "Cid Kagenou wants neither to be a hero nor a final boss, but to operate in the shadows. Pure chuunibyou comedy.",
  ...
  },
  {
    ...
    genre: "Isekai & Fantasy",
    description: "Momonga stays online as his favorite MMORPG shuts down, getting transported as an undead overlord with supreme power.",
  ...
  },
  {
    ...
    genre: "Isekai & Fantasy",
    description: "A high school girl is reborn as a weak dungeon spider and must survive perilous labyrinth beasts.",
  ...
  },
  {
    ...
    genre: "Isekai & Fantasy",
    description: "Summoned by a Goddess who calls him ugly and banishes him to the wasteland, Makoto builds a demi-human sanctuary.",
  ...
  },
  {
    ...
    genre: "Isekai & Fantasy",
    description: "Naofumi is summoned as one of four cardinal heroes, framed by royalty, and rises from betrayal.",
  ...
  },
  {
    ...
    genre: "Isekai & Fantasy",
    description: "Hiraku receives a healthy body and almighty farming tool from God, starting a peaceful village in a dark forest.",
  ...
  // Psychological & Mind Games
  {
    ...
    genre: "Psychological",
    description: "The direct sequel to COTE Year 1. Ayanokouji and Class D enter their second year facing new White Room enforcers.",
  ...
  },
  {
    ...
    genre: "Psychological",
    description: "Cynical loner Hachiman Hikigaya is forced into the Service Club to solve students' complicated social dilemmas.",
  ...
  },
  {
    ...
    genre: "Psychological",
    description: "On Academy Island, students duel for stars. Hiroto accidentally defeats the empress on day one and bluffs to the top.",
  ...
  },
  {
    ...
    genre: "Psychological",
    description: "Genius shut-in gamer siblings Sora and Shiro are summoned to Disboard, a world where everything is decided by games.",
  ...
  // Mystery & Historical
  {
    ...
    description: "Deep in the inner palace lives the Raven Consort, an enigmatic maiden solving court curses and spectral mysteries.",
  ...
  },
  {
    ...
    genre: "Mystery & Historical",
    description: "Selfish Princess Mia is guillotined during revolution, only to wake up as a 12-year-old with her bloodstained diary.",
  ...
  },
  {
    ...
    genre: "Mystery & Historical",
    description: "A free-spirited witch travels across strange nations, uncovering poignant, melancholic, and mysterious tales.",
  ...
  // Rom-Com & Slice of Life
  {
    ...
    description: "After lending an umbrella on a rainy day, Amane's neighbor Mahiru—the school's angel—starts cooking and caring for him.",
  ...
  },
  {
    ...
    genre: "Rom-Com & Slice of Life",
    description: "Yuuta and Saki become stepsiblings after parental remarriage, navigating emotional boundaries with quiet realism.",
  ...
  },
  {
    ...
    genre: "Rom-Com & Slice of Life",
    description: "Top-tier popular high-schooler Saku Chitose helps re-integrate a reclusive shut-in classmate into school hierarchy.",
  ...
  // Action & Dungeon Crawling
  {
    ...
    description: "A silver-ranked adventurer who refuses all noble quests to dedicate his existence to eradicating goblins with brutal tactics.",
  ...
  },
  {
    ...
    genre: "Action & Dungeon",
    description: "Betrayed and shoved into the deepest abyss of an Orcus labyrinth, Hajime transmutes monsters to survive and conquer.",
  ...
  },
  {
    ...
    genre: "Action & Dungeon",
    description: "The weakest E-rank hunter Sung Jin-woo awakens a unique quest system that allows him to level up without limits.",
    basedOn: "Danmachi, TenKen",
  }
];
 ...
 */
export function getSmartRecommendations(existingNovels: { title: string; url: string }[]): NovelRecommendation[] {
  // Normalize existing URLs and titles for fast lookup
  const existingUrls = new Set(
    existingNovels.map(n => (n.url || '').toLowerCase().trim().replace(/\/+$/, ''))
  );
  const existingTitles = existingNovels.map(n => (n.title || '').toLowerCase());
  ...
  const unScrapedCandidates = NOVEL_CATALOG.filter(candidate => {
    ...
    const isDuplicate = existingTitles.some(exist => 
      exist.includes(candTitleLower) || candTitleLower.includes(exist)
    );
    return !isDuplicate;
  });
  ...
  for (const n of existingNovels) {
    ...
    if (t.includes('slime') || t.includes('re:zero') || t.includes('isekai') || t.includes('tensei') || t.includes('tamer') || t.includes('seirei') || t.includes('death march') || t.includes('honzuki') || t.includes('bookworm') || t.includes('hero')) {
      genreScore['Isekai & Fantasy'] += 2;
    }
    if (t.includes('kyoushitsu') || t.includes('classroom') || t.includes('elite')) {
      genreScore['Psychological'] += 3;
    }
    if (t.includes('kusuriya') || t.includes('apothecary') || t.includes('witch') || t.includes('silent')) {
      genreScore['Mystery & Historical'] += 3;
    }
    if (t.includes('heroine') || t.includes('makeine') || t.includes('roshia') || t.includes('roshidere') || t.includes('alya')) {
      genreScore['Rom-Com & Slice of Life'] += 3;
    }
    if (t.includes('dungeon') || t.includes('danmachi') || t.includes('grimgar') || t.includes('ken')) {
