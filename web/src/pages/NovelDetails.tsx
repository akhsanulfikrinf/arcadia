...
export default function NovelDetails() {
    ...
    if (
        ...
        Object.keys(expandedVolumes).length === 0
    ) {
    ...
    const totalReadCount = chapters.filter((c) =>
        readHistory.includes(c.id),
    ).length;
    const percentRead =
        chapters.length > 0
            ? Math.round((totalReadCount / chapters.length) * 100)
            : 0;
    ...
    return (
        <div className="max-w-4xl mx-auto px-4 py-8 font-sans">
            <Link
                to="/"
                className="inline-flex items-center text-sm font-bold text-black dark:text-white hover:opacity-70 mb-6 transition"
            >
                            ...
                            {chapters.length > 0 && (
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-sm font-medium text-black/70 dark:text-white/70">
                                        {percentRead}% Read ({totalReadCount}/
                                        {chapters.length})
                                    </span>
                                </div>
                            )}
                        ...
                        {chapters.length > 0 && (
                            <div className="flex gap-3 mt-2">
                                {readHistory.length > 0 ? (
                                    <Link
                                        to={`/read/${readHistory[readHistory.length - 1]}`}
                                        className="bg-black text-white dark:bg-white dark:text-black px-8 py-3 rounded-xl font-medium hover:opacity-80 transition shadow-sm hover:shadow-md inline-block"
                                    >
                                        Continue Reading
                                    </Link>
                                ) : (
                ...
                <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm overflow-hidden border border-gray-100 dark:border-gray-700">
                    {chapters.length === 0 ? (
                        <div className="p-8 text-center text-gray-500">
                            No chapters have been scraped yet. Please check
                                    ...
                                    <div key={volName} className="volume-group">
                                        ...
                                        >
                                            <span className="font-bold text-black dark:text-white">
                                                {volName}{" "}
                                                <span className="text-black/50 dark:text-white/50 font-normal text-sm ml-2">
                                                    ({volChapters.length}{" "}
                                                    chapters)
                                                </span>
                                            </span>
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            <div className="flex items-center">
                                                                <span
                                                                    className={`w-12 text-sm font-mono text-black dark:text-white`}
                                                                >
                                                                    {isRead
                                                                        ? "✓ Read"
                                                                ...
                                                                <span
                                                                    className={`font-medium transition pl-2 ${isRead ? "text-black dark:text-white line-through" : "text-black dark:text-white"}`}
                                                                >
                                                                    {
                                                                        chapter.title
                                                                    }
