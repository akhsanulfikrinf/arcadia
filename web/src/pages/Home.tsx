    ...
    useEffect(() => {
        async function loadData() {
          ...
          `,
                ...
                if (historyData)
                    setRecentReads(historyData as unknown as RecentRead[]);
            }
        ...
        <div className="max-w-5xl mx-auto px-4 py-8">
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0 flex flex-col justify-center">
                                    ...
                                    <p className="text-xs text-black/50 dark:text-white/50 truncate">
                                        {read.chapters?.title.includes(
                                            "Chapter",
                                        )
                                            ? read.chapters?.title
                                            : `Ch. ${read.chapters?.chapter_index}`}
                                    </p>
                                </div>
                            </Link>
                        ))}
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                ...
                <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
                    ...
                    <Link
                        to="/admin"
                        className="px-4 py-2 bg-black text-white dark:bg-white dark:text-black rounded-xl text-sm font-bold hover:opacity-80 transition order-3 shrink-0"
                    >
                        Add
                    </Link>
                </div>
            ...
            {filteredNovels.length === 0 ? (
                <div className="text-center py-20 bg-white dark:bg-gray-800 rounded-3xl border border-dashed border-gray-200 dark:border-gray-700">
                    ...
                    {!searchQuery && (
                        <Link
                            to="/admin"
                            className="mt-4 inline-block bg-black text-white dark:bg-white dark:text-black px-6 py-2 rounded-lg hover:opacity-80 transition"
                        >
                            ...
                            <div className="bg-white dark:bg-gray-800 rounded-2xl overflow-hidden border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-lg transition">
                                <div className="aspect-[2/3] bg-gray-200 dark:bg-gray-900 relative">
                                    ...
                                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-end p-4">
                                        <span className="text-white font-bold text-sm bg-indigo-600 px-3 py-1 rounded-lg">
                                            View Novel
                                        </span>
                                    </div>
                                </div>
                                <div className="p-4">
                                    <h2
                                        className="font-bold text-black dark:text-white line-clamp-2 leading-tight"
                                        title={novel.title}
                                    >
                                        {novel.title}
                                    </h2>
