        ...
        try {
            ...
            if (bookmarks.includes(id)) {
                ...
                if (profileId) {
                    await supabase
                        .from("bookmarks")
                        .insert({ profile_id: profileId, chapter_id: id });
                }
            }
        } catch (e) {
            console.error(e);
    ...
    useEffect(() => {
        async function loadContent() {
            ...
            if (chData) {
                ...
                try {
                    const profileId =
                        localStorage.getItem("arcadia_profile_id");

                    // 1. Sync locally
                    const history = JSON.parse(
                        localStorage.getItem("arcadia_history") || "{}",
                    );
                ...
                try {
                    ...
                    const { data: blob, error: storageErr } =
                ...
                const { data: allChData } = await supabase
                    ...
                    .eq("novel_id", chData.novel_id)
                    .order("chapter_index", { ascending: true });

                if (allChData) {
                    setAllChapters(allChData);
                    const currentIndex = allChData.findIndex(
                        (c) => c.id === id,
                    );
                    if (currentIndex > 0)
                        setPrevChapter(allChData[currentIndex - 1]);
                    else setPrevChapter(null);

                    if (currentIndex < allChData.length - 1)
                        setNextChapter(allChData[currentIndex + 1]);
    ...
    if (loading)
        return (
            <div className="flex justify-center flex-col items-center h-screen bg-white dark:bg-[#121212]">
                <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-black dark:border-white mb-4"></div>
                <p className="text-black dark:text-white">Loading chapter...</p>
            </div>
        );
    if (!chapter)
        return (
            <div className="text-center py-20 text-red-500 bg-white dark:bg-[#121212] min-h-screen">
                Chapter not found.
            </div>
        );
        ...
        <div className="bg-white dark:bg-black min-h-screen font-sans pb-32 text-black dark:text-white transition-colors">
            <div className="max-w-4xl mx-auto px-6 pt-12">
                ...
                <div className="flex flex-wrap items-center text-black/60 dark:text-white/60 text-sm mb-10 gap-2 font-medium">
                    <Link
                        to="/"
                        className="hover:opacity-70 transition text-black dark:text-white"
                    >
                        Home
                    </Link>
                    <span>/</span>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-16 border-b border-gray-200 dark:border-gray-800 pb-8">
                    ...
                    <div className="flex items-center gap-4 flex-wrap">
                        <div className="flex items-center gap-2">
                            <button
                                onClick={() =>
                                    setFontSize((f) => Math.min(32, f + 2))
                                }
                                className="w-10 h-10 rounded-full border-2 border-black dark:border-white bg-transparent flex justify-center items-center text-black dark:text-white hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black transition"
                            >
                            ...
                            <button
                                ...
                                className={`w-10 h-10 rounded-full border-2 border-black dark:border-white flex justify-center items-center transition ${isBookmarked ? "bg-black text-white dark:bg-white dark:text-black" : "bg-transparent text-black dark:text-white hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black"}`}
                            >
                                <Bookmark
                        ...
                        <div className="flex items-center gap-2">
                            {prevChapter ? (
                                ...
                                <button
                                    disabled
                                    className="border-2 border-gray-300 dark:border-gray-700 text-gray-400 font-bold rounded-lg px-4 py-2.5 flex items-center cursor-not-allowed"
                                >
                                    <ChevronLeft className="w-5 h-5" /> Prev
                                </button>
                            )}
                            ...
                            {nextChapter ? (
                                ...
                                <button
                                    disabled
                                    className="bg-gray-300 dark:bg-gray-700 border-2 border-gray-300 dark:border-gray-700 text-gray-500 font-bold rounded-lg px-4 py-2.5 flex items-center cursor-not-allowed"
                                >
                                    Next <ChevronRight className="w-5 h-5" />
                                </button>
                            )}
                ...
                >
                    <div className="space-y-6">
                        {(() => {
                            ...
                            const firstTextBlockIndex = contents.findIndex(
                                (b) => {
                            ...
                            return contents.map((block, index) => {
                                ...
                                if (block.type === "title") {
                                    return (
                                        <h3
                                            key={block.id}
                                            className="font-bold mt-12 mb-6 text-black dark:text-white"
                                            style={{
                                ...
                                if (index === firstTextBlockIndex) {
                                    ...
                                    const splitMatch = textStr.match(
                                        /^(.{3,80}?[.!?:])(?:\s+([A-Z"“].*))/s,
                                    );
                                    ...
                                    if (splitMatch) {
                                        ...
                                        return (
                                            <p
                                                key={block.id}
                                                className="text-left mb-6 text-black dark:text-white"
                                            >
                                                <span
                                                    className="block font-bold text-black dark:text-white mb-2"
                                                    style={{
                                                        fontSize: `${fontSize * 1.3}px`,
                                                    }}
                                                >
                                ...
                                const babRegex =
                                    /^((?:Bab|Chapter)\s+\d+(?:[\s:-]+[A-Z][a-zA-Z0-9\s]*?)?)(?:(?=[.!?]|[\r\n]|\s+[A-Z“"'])|$)/i;
                                const match = textStr.match(babRegex);

                                if (match && match[1]) {
                                    const prefix = match[1].trim();
                                    const remainder = textStr
                                    ...
                                    if (remainder.length === 0) {
                                        return (
                                            <p
                                                key={block.id}
                                                className="text-left mb-6 mt-12 text-black dark:text-white"
                                            >
                                                <span
                ...
                <div className="mt-20 flex justify-between items-center pt-8 border-t border-gray-100 dark:border-gray-800">
                    ...
                    {nextChapter ? (
                        ...
                        <span className="text-black/60 dark:text-white/60 italic font-medium">
