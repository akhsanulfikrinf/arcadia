...
export default function Bookmarks() {
  ...
  useEffect(() => {
    async function loadBookmarks() {
      ...
      let bookmarkIds = JSON.parse(localStorage.getItem('arcadia_bookmarks') || '[]')
  ...
  if (loading) return (
    <div className="flex justify-center flex-col items-center h-screen bg-white dark:bg-[#121212]">
      ...
      <p className="text-black dark:text-white font-medium">Loading bookmarks...</p>
    </div>
  )
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <div className="mb-10">
        ...
        <p className="text-black/70 dark:text-white/70">Your saved chapters for quick access.</p>
      </div>
      ...
      {bookmarkedChapters.length === 0 ? (
        <div className="text-center py-20 bg-white dark:bg-gray-800 rounded-3xl border border-dashed border-gray-200 dark:border-gray-700">
          ...
          <p className="text-black/50 dark:text-white/50">You haven't bookmarked any chapters yet.</p>
          <Link to="/" className="mt-4 inline-block px-6 py-2 bg-black text-white dark:bg-white dark:text-black rounded-xl font-bold hover:opacity-80 transition">Explore Library</Link>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-4">
          {bookmarkedChapters.map((ch) => (
            <Link key={ch.id} to={`/read/${ch.id}`} className="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700 hover:shadow-md transition group flex items-center gap-4">
              <img src={ch.novels?.cover_url} alt="" className="w-12 h-16 object-cover rounded-lg bg-gray-100 dark:bg-gray-900 shrink-0" />
              <div className="flex-1 min-w-0">
                <p className="text-xs font-bold text-black/50 dark:text-white/50 uppercase tracking-wider mb-1 truncate">{ch.novels?.title}</p>
                <h3 className="font-bold text-black dark:text-white truncate">{ch.title}</h3>
              </div>
              <button 
