...
export default function History() {
    ...
    useEffect(() => {
        async function loadHistory() {
            ...
            const historyMap: Record<string, string[]> = JSON.parse(
            ...
            if (profileId) {
                ...
                if (dbHistory) {
                    dbHistory.forEach((item) => {
                        if (!historyMap[item.novel_id])
                            historyMap[item.novel_id] = [];
                        if (
                            !historyMap[item.novel_id].includes(item.chapter_id)
                        ) {
            ...
            setHistoryItems(
                formattedHistory.filter((h): h is HistoryItem =>
                    Boolean(h.lastChapter),
                ),
            );
            setLoading(false);
        }
                    ...
                    {historyItems.map((item) => (
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                >
                                    <h3 className="font-bold text-xl text-black dark:text-white mb-1 group-hover:text-black/70 dark:group-hover:text-white/70 transition truncate">
                                        {item.title}
                                    </h3>
                                </Link>
                                <p className="text-sm text-black/50 dark:text-white/50 mb-4">
                                    You have read {item.totalRead} chapters
                                </p>
