...
export default function ManualRescrape() {
            ...
            if (!novelError && novelData) {
                const novelsWithStats = await Promise.all(
                    novelData.map(async (n) => {
                        ...
                        const { data: completedData } = await supabase.rpc(
                            "get_completed_chapters_count",
                            { n_id: n.id },
                        );
                        const completedCount = completedData || 0;

                        const { data: latestCh } = await supabase
                            .from("chapters")
                            .select("created_at")
                            .eq("novel_id", n.id)
    ...
    const handleRescrape = async (novelUrl: string, novelId: string) => {
        if (
            !confirm(
                `Are you sure you want to re-scrape this novel? This will update all chapters and illustrations.`,
            )
        ) {
            return;
        }
        ...
        try {
            ...
            setStatus({
                ...
                message:
                    data?.message ||
                    "Scraper triggered! Chapters will be refreshed soon.",
            });
        ...
        <div className="max-w-4xl mx-auto px-4 py-12">
            ...
            <div className="mb-10">
                ...
                <p className="text-black/70 dark:text-white/70">
                    Select a specific novel to force a full re-scrape of its
            ...
            {status.message && (
                <div
                    className={`mb-8 p-4 rounded-xl flex items-center border ${
                        status.type === "error"
                ...
                >
                    {status.type === "success" ? (
                        ...
                        <RefreshCw
                            className={`w-5 h-5 mr-3 shrink-0 ${status.type === "loading" ? "animate-spin" : ""}`}
                        />
                    )}
            ...
            <div className="grid grid-cols-1 gap-4">
                {filteredNovels.map((novel) => (
                    <div
                        key={novel.id}
                        className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-100 dark:border-gray-700 hover:shadow-md transition"
                    >
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-6">
                                ...
                                <div className="min-w-0">
                                    ...
                                    <div className="flex flex-wrap items-center gap-2">
                                        <span className="px-2 py-0.5 bg-black text-white dark:bg-white dark:text-black text-[10px] font-bold rounded uppercase tracking-wider">
                                            ...
                                            {novel.chapterCount} Scraped
                                        </span>
                                        {(() => {
                                            const statusInfo = getScrapeStatus(
                                                novel.completedCount,
                                                novel.chapterCount,
                                            );
                                            return (
                                                <span
                                                    className={`px-2 py-0.5 border text-[10px] font-bold rounded uppercase tracking-wider ${statusInfo.color}`}
                                                >
                                                    {statusInfo.label}
                                                </span>
                                            );
                            ...
                            <button
                                onClick={() =>
                                    handleRescrape(novel.url, novel.id)
                                }
                        ...
                        <div className="w-full h-2 bg-gray-100 dark:bg-gray-900 rounded-full overflow-hidden">
                            <div
                                className={`h-full transition-all duration-1000 ${
                                    ...
                                    novel.completedCount === novel.chapterCount
                                        ? "bg-green-500"
                                        : novel.completedCount > 0
                                          ? "bg-blue-500"
                                          : "bg-red-500"
                                }`}
                                style={{
                                    width:
                                        novel.chapterCount > 0
                                            ? `${(novel.completedCount / novel.chapterCount) * 100}%`
                                            : "0%",
                                }}
                            />
                        </div>
